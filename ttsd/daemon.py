...
and multi-engine TTS generation.
"""
...
from app import TTSConfig, TTSEngine, AudioPlayer, ContentExtractor, CACHE_DIR
...
class TTSDaemon:
    ...
    def submit_job(self, text: str, options: Optional[Dict[str, Any]] = None) -> int:
        ...
        cmd = Command(action="speak", text=text, options=options or {}, job_id=job_id)
    ...
    def get_state(self) -> Dict[str, Any]:
        ...
        with self._lock:
            state_info = {
                ...
                "current_job": None,
                "jobs_pending": len([j for j in self.jobs.values() if j.status == "queued"])
            }
